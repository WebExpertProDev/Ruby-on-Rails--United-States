...
module Liquid    
    ...
    module UI
        ...
        # @return {Hash} node   
        #                           
        def get_liquid_method_tree(cfg = {})
            ...
            rs.each do |m|                    
                ...
                if (assn)                                      
                    if child = assn.klass.get_liquid_method_tree(cfg)                                                                                        
                        child[:iconCls] = cfg[assn.macro] || Liquid::UI.const_get("ICON_" + assn.macro.to_s.upcase)                                                                      
                        child[:text] = m
                    ...
                    node[:children].push({                        
                        :text => m,
                        :iconCls => cfg[:tag] || Liquid::UI::ICON_TAG,
                        :leaf => true
                    })
                end
            end
...
ActiveRecord::Base.send(:extend, Liquid::UI)
