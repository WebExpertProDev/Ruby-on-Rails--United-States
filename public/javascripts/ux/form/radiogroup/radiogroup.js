...
Ext.ux.RadioGroup = Ext.extend(Ext.form.Field,  {
    ...
    /**
     * @cfg {String} labelCls [''] A CSS class to add to label
     * @author Chris Scott
     */
    labelCls: '',
    ...
    onRender : function(ct, position){
        ...
        if(typeof(this.boxLabel) != 'undefined'){
            var labelCls = 'x-form-cb-label';
            if (typeof(this.labelCls) != 'undefined') {
                labelCls += ' ' + this.labelCls;
            }
            this.wrap.createChild({tag: 'label', htmlFor: this.el.id, cls: labelCls, html: this.boxLabel});
        }
		if(!this.isInGroup){
		...
		if (this.radios && this.radios instanceof Array) {
			...
			for(var i=1;i<this.radios.length;i++){
				...
				this.els[i]=new Ext.ux.RadioGroup({
					...
					tabIndex:r.tabIndex,
					cls:r.cls,
                    labelCls: r.labelCls,     // <-- added by Chris to support icons on labels.
    ...
    getByValue : function(v) {
        ...
        for (var n=0,len=this.els.length;n<len;n++) {
            if (this.els[n] instanceof Ext.ux.RadioGroup && this.els[n].value == v) {
                return this.els[n];
            }    
        }
        return false;
    ...
    disableAll : function() {
        ...
        for (var n=0,len=this.els.length;n<len;n++) {            
            if (this.els[n] instanceof Ext.ux.RadioGroup) { this.els[n].disable(); this.els[n].setChecked(false);}                                
        }
    }
});
